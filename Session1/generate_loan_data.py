...
def generate_loan_row(n):
    ...
    for i in range(n):
        ...
        coapplicant_income = int(rng.choice([0, rng.normal(2000,1500)], p=[0.6, 0.4]))
        # loan amount correlated with income
        loan_amount = max(20, int(np.clip(rng.normal(applicant_income*0.2 + coapplicant_income*0.1, 50), 20, 600)))
        loan_amount_term = int(rng.choice([120,180,240,360], p=[0.05,0.1,0.15,0.7]))
        ...
        # Make label with some realistic logic (probability of approval)
        # Higher income, credit_history=1, small loan -> higher approval probability
        score = 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_rows", type=int, default=100_000, help="number of rows to generate")
